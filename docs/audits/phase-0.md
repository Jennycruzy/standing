...
Phase 0 does not pass. It can pass only after team registration is confirmed; two Virtuals ACP agents are registered with deployed wallets and complete the published self-evaluation lifecycle with funded buyer USDC; a funded dedicated Base wallet performs a real schema registration, attestation, reference-chain read, ERC-8004 identity registration, and reputation write/read (or verifies that the fallback is necessary); an exact extraction and reviewer model identifier is verified; and the archive restore path is resolved.
