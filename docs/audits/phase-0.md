...
5. Version 3 makes a completed ACP sandbox lifecycle the earliest hard gate. The lifecycle now passes through the current ACP v2 path; the remaining gate is an explicit decision about the Python-throughout requirement because the current official ACP v2 client is TypeScript/Node and the installed Python client is a legacy Entity-ID/EOA client.
...
Phase 0 does not pass yet. Do not revisit the dead `/acp/join` link, create duplicate agents, or invent Entity IDs. The ACP v2 path is verified by a completed Base job; the remaining hard-stop items are the Sibyl archive restore path, funded Base/EAS and ERC-8004 checks, model identifier, hackathon registration, and the explicit choice between strict Python-only implementation and a small official TypeScript ACP adapter. No Phase 1 product code should be started until that language decision is recorded.
