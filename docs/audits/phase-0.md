...
8. The current portal separates the routes: `https://app.virtuals.io/acp/agents` is the EconomyOS “My Agents” page shown in the screenshot, while the top navigation’s ACP entry opens `https://app.virtuals.io/acp/scan`. The official setup documentation places the Entity ID in the Wallet Management → Whitelisted Wallets table, after creating the smart-contract account and whitelisting the developer wallet. The official CLI’s open issue #92 documents the same opaque HTTP 429 behavior, with no published reset interval.

## Exit criteria

Phase 0 does not pass. Next action is to use the current ACP route (`/acp/scan`, not the token-launch page and not the EconomyOS “My Agents” page) to register the Requestor and Provider, create each smart-contract account, whitelist the developer wallet, and copy the numeric Entity IDs from Wallet Management. It can then pass after the two agents complete the published self-evaluation lifecycle with funded buyer USDC; a funded dedicated Base wallet performs a real schema registration, attestation, reference-chain read, ERC-8004 identity registration, and reputation write/read (or verifies that the fallback is necessary); an exact extraction and reviewer model identifier is verified; and the archive restore path is resolved.
