...
The first claim is demonstrated by the live jobs in [the verifier-loop audit](verifier-loop.md). The second is intentionally not claimed by the current controlled demo.

Before a non-demo release, [`standing/release.py`](../../standing/release.py) requires:

- a visible controlled-demo disclosure whenever sandbox data is shown;
